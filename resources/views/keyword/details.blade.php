                            ...
                            <div class="mb-3 mb-md-0" style="display:flex; align-items:center; gap:10px;">
                                ...
                                <div>
                                    <div class="input-group">
                                        <input type="text" readonly name="daterange" class="form-control"
                                            id="dateRangeInput" placeholder="Select Date Range"
                                ...
                                <div class="form-row align-items-center">
                                    @role('Admin|Super Admin')
                                        ...
                                        <div class="col">
                                            <select name="users[]" id="field1" class="form-control" multiple
                                                multiselect-search="true" multiselect-max-items="3">
                                    ...
                                    <div class="col">
                                        <select name="labels[]" id="field2" class="form-control" multiple
                                            multiselect-search="true" multiselect-max-items="3">
                                ...
                                <input type="hidden" name="positionFilter" id="positionFilter"
                    ...
                    <div class="mb-4 position-filters">
                        <h6>
                            <img src="https://flagcdn.com/32x24/{{ strtolower($country->code) }}.png"
                                alt="{{ $country->name }} flag" class="me-1">
                            {{ $country->name }}
                        </h6>
                            ...
                            @foreach ($ranges as $range)
                                @php
                                    ...
                                    if ($range === 'all') {
                                        ...
                                        $increaseCount =
                                            $countryRanges[$country->id][$range]['keywords_position_increase_count'] ??
                                            0;
                                        $decreaseCount =
                                            $countryRanges[$country->id][$range]['keywords_position_decrease_count'] ??
                                            0;
                                    }
                                @endphp
                                <div class="country_filter">
                                    ...
                                    <button type="button"
                                        class="position-filter-btn btn btn-light border flex-shrink-0 {{ $positionFilter == $range && $country->id == $selectedCountry ? 'active' : '' }}"
                                        onclick="applyFilter('{{ $country->id }}', '{{ $range }}')">
                                    ...
                                    <strong>
                                        ...
                                        @if ($range !== 'all')
                                            @if ($increaseCount > $decreaseCount)
                                                <span class="text-success">▲ {{ $increaseCount }}</span>
                                            @elseif ($decreaseCount > $increaseCount)
                                                <span class="text-danger">▼ {{ $decreaseCount }}</span>
                                            @endif
                                        @endif
                                    </strong>
                                </div>
        ...
        <div class="filter-country-main">
            ...
            <h5>
                <img src="https://flagcdn.com/32x24/{{ strtolower($countries->firstWhere('id', $selectedCountry)->code) }}.png"
                    alt="{{ $countries->firstWhere('id', $selectedCountry)->name }} flag" class="me-1">
                        ...
                        @foreach ($keywordData as $data)
                            <tr>
                                ...
                                @foreach ($allDates as $index => $date)
                                    @php
                                        $positionData = $data['positions'][$date] ?? [
                                            'position' => '-',
                                            'clicks' => '-',
                                            'impressions' => '-',
                                        ];
                                        $currentPosition = is_numeric($positionData['position'])
                                            ? (int) $positionData['position']
                                            : $positionData['position'];
                                        $previousPosition =
                                            $index > 0
                                                ? $data['positions'][$allDates[$index - 1]]['position'] ?? '-'
                                                : '-';
                                        $positionChange =
                                            is_numeric($currentPosition) && is_numeric($previousPosition)
